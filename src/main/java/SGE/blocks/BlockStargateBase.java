...
import SGE.items.ItemBlockStargateBase;
...
public class BlockStargateBase extends SGEBlock {
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float cx,
		...
		if (te != null && te.getState() == MultiblockState.FORMED) {
			player.openGui(SGEvolution.instance, SGERuntime.runtime.interfaces().stargateUI.getGUIID(), world, x, y, z);
			return true;
		}
		return false;
	}
